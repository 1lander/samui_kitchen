          ...
          >
            <div class="flex justify-between items-start">
              <h3 class="text-xl font-semibold">
                {{ t(item.name) }}
                <span
                  v-if="item.dishChoices && item.dishChoices.length > 0"
                  class="text-sm text-gray-600 block mt-1"
                >
                  {{ t("menu.dishChoices.choice") }}:
                  <span
                    v-for="(choice, choiceIndex) in item.dishChoices"
                    :key="choiceIndex"
                    class="mr-1"
                  >
                    {{ t(`menu.dishChoices.${choice}`) }}
                    <template v-if="choiceIndex < item.dishChoices.length - 1"
                      >/</template
                    >
                  </span>
                </span>
